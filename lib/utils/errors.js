...
export const createTagError = (error, tag, operation) => {
  ...
  if (tag?.attrs && tag.attrs.length > 0) {
    context.attributes = tag.attrs.map(attribute => `${attribute.name}="${attribute.value}"`).join(' ');
  }
...
export const createParseError = (message, content, position) => {
  ...
  const lines = content.slice(0, Math.max(0, position)).split('\n');
 ...
 */
export const wrapWithErrorHandling = (function_, operation, context = {}) => {
  return (...arguments_) => {
    try {
 ...
 */
export const wrapAsyncWithErrorHandling = (function_, operation, context = {}) => {
