...
export const getTheme = (customThemeInput = {}) => {
  ...
  // Input styles - button
  const buttonPrefixStyle = parseStyleEntry(
    customTheme.input?.button?.prefix,
    baseTheme.input?.button?.prefix,
    ["marker"],
  );
  const buttonSuffixStyle = parseStyleEntry(
    customTheme.input?.button?.suffix,
    baseTheme.input?.button?.suffix,
    ["marker"],
  );
  ...
  const createHeaderTheme = (style) => {
    ...
    const indicatorColorFn = style.indicator?.color
  ...
  return {
    ...
    blockquote: (() => {
      const style = parseStyleEntry(customTheme.blockquote, baseTheme.blockquote, ["indicator"]);
      const indicatorColorFn = style.indicator?.color
        ? (text) => chalkString(style.indicator.color, forceColor ? { colors: true } : undefined)(text)
        : null;
      return {
    ...
    input: {
      ...
      button: {
        prefix: buttonPrefixStyle,
        suffix: buttonSuffixStyle,
