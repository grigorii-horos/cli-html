...
// Inlined to avoid the `number-to-alphabet` dependency, whose parcel UMD
// bundle has no real ESM export and breaks under Deno (issue #14).
...
export const getListItemNumber = (number = 1, type = '1') => {
  const types = {
    1: `${number}`,
    A: numberToAlphabet(number).toUpperCase(),
    a: numberToAlphabet(number),
    I: romanize(number),
