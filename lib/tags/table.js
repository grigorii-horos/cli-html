...
const captions = (tag, context) =>
  blockTag((value) => bold.blue(value))(tag, context);
...
const trVals = (tr) => {
  ...
  const theadTdsValue = !theadTds
    ...
    : theadTds.map((tag, context) => {
        ...
        const rowSpan = parseInt(getAttribute(tag, "rowspan", "1"), 10);
...
export const table = (tag, context) => {
  ...
  const trs = tag.childNodes.filter((child) =>
    ["tbody"].includes(child.nodeName)
  );
