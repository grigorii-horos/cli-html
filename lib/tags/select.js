...
export const select = (tag, context) => {
  ...
  const selectedMarker = custom.selected.marker || optionTheme.selected?.marker || '◉';
  const unselectedMarker = custom.unselected.marker || optionTheme.unselected?.marker || '○';

  // Colors from theme are functions, from custom are strings
  const selectedColorString = custom.selected.color; // string or null
  const unselectedColorString = custom.unselected.color; // string or null
  const optionColorFunction = custom.option.color
    ? (text) => chalkString(custom.option.color, { colors: true })(text)
  ...
  const renderOption = (optionTag, indent = '', parentDisabled = false) => {
    ...
    let styledMarker;
    if (isSelected) {
      styledMarker = selectedColorString
        ? chalkString(selectedColorString, { colors: true })(marker)
        : (optionTheme.selected?.color ? optionTheme.selected.color(marker) : marker);
    } else {
      styledMarker = unselectedColorString
        ? chalkString(unselectedColorString, { colors: true })(marker)
    ...
    let styledLabel;
    if (isDisabled) {
      const disabledColor = optionCustom.disabled?.color || context.theme.option?.disabled?.color || 'gray dim';
      styledLabel = chalkString(disabledColor, { colors: true })(displayLabel);
    } else if (optionCustom.color) {
      styledLabel = chalkString(optionCustom.color, { colors: true })(displayLabel);
    } else if (optionColorFunction && typeof optionColorFunction === 'function') {
      styledLabel = optionColorFunction(displayLabel);
    } else {
  ...
  if (selectName) {
    const selectLabel = custom.select.label || selectName;
