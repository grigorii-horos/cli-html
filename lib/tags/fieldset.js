...
export const fieldset = (tag, context) => {
  ...
  const dimBorder = custom.borderDim !== null ? custom.borderDim : (context.theme.fieldset?.border?.dim ?? false);
  const titleColorRaw = custom.titleColor || context.theme.fieldset?.title?.color || 'yellow';
  ...
  return blockTag(
    (value) => {
      ...
      return isDisabled ? chalkString('gray dim', { colors: true })(boxedValue) : boxedValue;
    },
