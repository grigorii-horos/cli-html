...
export const details = (tag, context) => {
  ...
  const dimBorder = custom.borderDim === null ? (context.theme.details?.border?.dim ?? false) : custom.borderDim;
  ...
  if (typeof indicatorConfig === 'object') {
    ...
    const openMarker = custom.open.marker
      || (typeof openConfig === 'object' ? openConfig.marker : openConfig)
      || '▼ ';
    const closedMarker = custom.closed.marker
