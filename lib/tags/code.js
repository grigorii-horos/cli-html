...
export const code = (tag, context) => inlineTag((value, tag) => {
  ...
  const codeValue = langName
    ? highlight(content, {
      language: langName,
      theme: {
        comment: ansiColors.grey.italic,
      },
    })
    : ansiColors.yellow(content);

  const codeValueLines = codeValue.split('\n');

  if (codeValueLines.length <= 1) {
    return codeValue;
  }
  ...
  const codeContent = codeValueLines.map(
    (codeLine, index) => `${ansiColors.grey.dim(`${index + 1}`.padStart(codeLinesLength, ' '))} ${indentify(
      pad,
      true,
    )(
...
)(tag, { ...context, pre: true, lineWidth: context.lineWidth - 8 });
